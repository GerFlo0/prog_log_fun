...
# Crear cabecera
header = [tk.Label(root) for _ in range(len(HEADERS))]
...
btn_guardar = ttk.Button(root, text="Guardar entrada", command=lambda: procesar_entrada(selected_bus, selected_destino, entry_corridas))

btn_terminar = ttk.Button(root, text="Terminar", command=lambda: terminar(tabla, entradas))
